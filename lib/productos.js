...
//
// Reglas automaticas del catalogo:
// - precio: SIEMPRE usar redondear99() al definirlo, asi cualquier precio
//   final (precio + envio) termina en 99 en todo el sitio, sin excepcion.
// - stock: numero real de Dropi (revisar cada tanto, no se actualiza solo).
//   Si baja de UMBRAL_STOCK_BAJO, el sitio muestra "Ultimas unidades" solo.
// - El producto marcado como mas nuevo es siempre el ULTIMO del array.
//   No hace falta tocar nada: al agregar un producto nuevo al final,
//   el badge "Nuevo" se mueve solo a el.

export const UMBRAL_STOCK_BAJO = 20;

// Redondea cualquier precio a que termine en 99 (ej: 37900 -> 37899).
export function redondear99(precio) {
    return Math.floor(precio / 100) * 100 - 1;
}
...
// El producto "nuevo" es siempre el ultimo agregado al array.
export function esProductoNuevo(slug) {
    return productos[productos.length - 1].slug === slug;
}

// true si el stock real esta por debajo del umbral configurado arriba.
export function tieneStockBajo(producto) {
    return typeof producto.stock === "number" && producto.stock < UMBRAL_STOCK_BAJO;
}
